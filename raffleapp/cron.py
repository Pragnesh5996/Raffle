...
from .models import Raffle, Product_Raffle, Subscribers
import json
...
class MyCronJob(CronJobBase):
    ...
    def do(self):
        check_raffle = Raffle.objects.all()

        for x in check_raffle:
            if (x.start_date.strftime("%Y-%m-%d %H:%M") == datetime.now().strftime("%Y-%m-%d %H:%M")):
                update_raffle = Raffle.objects.filter(id=x.id, is_active=True).update(is_new=True)
                print('yes')
            else:
                pass

class SelectWinner(CronJobBase):
    RUN_EVERY_MINS = 1 if settings.DEBUG else 1   # 6 hours when not DEBUG

    schedule = Schedule(run_every_mins=RUN_EVERY_MINS)
    code = 'cron.SelectWinner'    # a unique code
            ...
            if (x.end_date.strftime("%Y-%m-%d %H:%M") == datetime.now().strftime("%Y-%m-%d %H:%M")):
                ...
                if check_raffale_variant:
                    ...
                    if x.is_automatic == True: # for automatic #
                        ...
                        SendVendorEmail(url=url, vendor_email='pragnesh2612raval@gmail.com')